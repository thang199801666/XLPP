...
private:
    ...
    void moveFrom(ZipEntrySource&& other) noexcept;
    void open();
    bool refillInput();
    ...
    std::uint64_t producedUncompressed_{0};
    bool opened_{false};
    bool eof_{false};
...
public:
    ...
    ZipArchiveReader(const std::filesystem::path& path, const ZipArchiveReaderLimits& limits);
