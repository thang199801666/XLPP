...
public:
    ...
    // Serialize the complete ZIP package directly to memory. This is used by
    // encrypted Office-package writes so the plaintext inner OOXML ZIP never
    // needs to be materialized as a temporary file.
    std::vector<unsigned char> saveToBytes() const;
    std::vector<unsigned char> saveToBytes(const ZipWriteOptions& options) const;
    ...
    // Open a complete ZIP package already held in memory. The same safety
    // limits as the filesystem path are enforced.
    static ZipArchive open(const std::vector<unsigned char>& bytes);
    static ZipArchive open(const std::vector<unsigned char>& bytes, const ZipOpenLimits& limits);
...
private:
    void saveToStream(std::ostream& out, const ZipWriteOptions& options) const;
    struct Entry {
        std::string data;
