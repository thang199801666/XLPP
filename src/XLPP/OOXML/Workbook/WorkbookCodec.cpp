...
using xlpp::internal::ooxml::parseFilterOperator;
using xlpp::internal::ooxml::parseConditionalOperator;
using xlpp::internal::ooxml::parseCfvo;
using xlpp::internal::ooxml::parseDataValidationType;
using xlpp::internal::ooxml::parseDataValidationOperator;
using xlpp::internal::ooxml::parseDataValidationErrorStyle;
...
using xlpp::internal::ooxml::quotePivotSheetName;
...
void loadImportedPivotModels(xlpp::Worksheet& sheet,
    ...
    for (const auto& relationship : relationshipsForSource(relationships, sourceSheetPart)) {
        ...
        if (!cachePart.empty() && archive.contains(cachePart)) {
            ...
            if (const auto definitions = xlpp::internal::tags(cacheXml, "pivotCacheDefinition"); !definitions.empty()) {
                pivot.cache().setRefreshOnLoad(attrBool(definitions.front(), "refreshOnLoad", false));
                pivot.cache().setSaveData(attrBool(definitions.front(), "saveData", true));
                pivot.cache().setEnableRefresh(attrBool(definitions.front(), "enableRefresh", true));
            }
        ...
        if (const auto data = xlpp::internal::tags(definition, "dataFields"); !data.empty()) {
            for (const auto& node : xlpp::internal::tags(data.front(), "dataField")) {
                ...
                try {
                    ...
                    if (const auto showDataAs = xlpp::internal::attribute(node, "showDataAs"); !showDataAs.empty()) added.setShowDataAs(showDataAs);
...
void Workbook::saveInPlace(const std::filesystem::path& p, const SaveOptions& options) const {
    ...
    if (!options.encryptionPassword.empty()) {
        ...
        try {
            ...
            auto package = readBinaryFile(temporary);
    ...
    for (std::size_t i = 0; i < sheets_.size(); ++i) {
        ...
        if (!preservePivot[i] && !originalSheetPart.empty()) {
            for (const auto& relationship : originalSheetRelationships) {
                ...
                if (!pivotPart.empty()) suppressExclusivePartClosure(pivotPart, preservedRelationships_, suppressedPreservedParts);
            }
        }
...
void Workbook::loadInPlace(const std::filesystem::path& p, const LoadOptions& options) {
    ...
    // loading pipeline, including its entry/size/cancellation limits.
    {
        ...
        if (internal::hasCompoundFileSignature(signature.data(), static_cast<std::size_t>(input.gcount()))) {
            ...
            const auto compound = readBinaryFile(p);
            ...
            try {
                writeBinaryFile(temporary, package);
                auto plainOptions = options;
...
for(auto&s:internal::tags(wb,"sheet")) {
    ...
    for (std::size_t i = 0; i < sheets_.size() && i < sourceSheetParts_.size(); ++i)
        loadImportedPivotModels(sheets_[i], z, sourceSheetParts_[i], preservedRelationships_);
