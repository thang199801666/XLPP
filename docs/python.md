...
The Python binding now exposes P0X calculation/structural/encryption surfaces, including `Workbook.calculate_formulas()`, workbook-level structural edits, `SaveOptions` encryption profiles/key sizes, encryption inspection and the formula dependency graph. Dynamic-array and iterative-calculation report fields are exposed as well. The source binding requires pybind11/Python development dependencies to compile; the native C++ and C ABI gates do not substitute for a Python-extension build in release CI.
