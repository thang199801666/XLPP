...
| Automatic chart-cache dependency tracking + style-resource resolution — LibreOffice/OpenPyXL-derived fixture | PASS for generated multi-chart dependency targeting | PASS for dependency graph, changed-range matching, formula-cache fallback, style/color-style IDs/method/colors and theme font placeholder resolution | PASS | PASS | PASS for untouched chart-style/color-style resources and sibling drawing/media relationships | PASS via LibreOffice headless; Calc may remove Office chart-style/color-style extension resources on re-save | P0V dependency/style-resolution foundation PASS |
...
### P0W — formula dependencies and chart color styles

| Capability | XL++ direct | OpenPyXL reopen | LibreOffice re-save |
|---|---|---|---|
| Simple A1 formula-precedent dependency detection | Supported | N/A | N/A |
| Preserve stale formula chart cache + request recalc | Supported | Flags/formulas readable | Package remains valid; headless conversion may retain existing cached formula values |
| Theme-resolved color-style application | Supported | Applied SRGB formatting readable | Applied SRGB formatting preserved in P0W fixture |
| Full Excel formula calculation engine | Not provided | N/A | Host-dependent |
| Full Excel chart-style matrix/effect engine | Not yet | N/A | Host-dependent |
